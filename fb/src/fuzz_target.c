...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
  ...
  char *args[] = {"doom_fuzz",
                  ...
                  "-nogui", "-nosound", "-nosfx", "-nomusic", "-nomusicpacks", "-mmap",
                  NULL};
  ...
  char *filename2 = "~fuzz.txt";
  D_IsIWADName(filename);
  D_TryFindWADByName(filename);
  D_FindAllIWADs(0);
  I_StartTextInput(0, 0, 1, 1);
  I_StopTextInput();
  I_ReadMouse();
  DEH_OpenFile(filename);
  ...
  NetUpdate();
  D_GameModeString(gamemode);
  D_GameMissionString(gamemission);
  D_IsEpisodeMap(gamemission);
  D_ValidGameVersion(gamemission, gameversion);
  D_GetNumEpisodes(gamemission, gamemode);
  D_ValidEpisodeMap(gamemission, gamemode, 6, 1);
  D_ValidGameMode(gamemission, gamemode);
  M_Random();
  P_SubRandom();
  D_DoomMain();
  NET_SV_Run();
